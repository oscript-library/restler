...
Функция ПолучитьПоКлючевымПолям(ВыражениеКлючевыхПолей) Экспорт
	АдресРесурса = ПолучитьПубликациюОбъектаOData() 
		+ СтрШаблон("(%1)$format=json%2",ВыражениеКлючевыхПолей, ТекущееВыражениеФильтра);
	Лог.Отладка("Запрашиваю ресурс: " + АдресРесурса);
	Возврат КлиентВеб.Получить(АдресРесурса);
КонецФункции

Функция ПолучитьСсылку(СсылкаGUID) Экспорт
	ТекущееВыражениеФильтра = "&$top=1";
	Возврат ПолучитьПоКлючевымПолям("guid('" + СсылкаGUID + "')");
КонецФункции
...
Функция СоздатьЭлементСправочника(Знач Наименование, Знач Реквизиты) Экспорт
	...
	АдресРесурса = ПолучитьПубликациюОбъектаOData();
	Возврат КлиентВеб.Отправить(АдресРесурса);
КонецФункции
